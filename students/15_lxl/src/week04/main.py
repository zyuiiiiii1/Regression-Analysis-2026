...
def run_experiment():
    ...
    for exp_name, (N, P) in experiments.items():
        ...
        for solver_name, solver in custom_solvers.items():
            print(f"  Fitting with {solver_name}Solver...")
            solver.fit(X[:, 1:], y)  # Exclude intercept column for fitting since solver handles it internally
            y_pred = solver.predict(X[:, 1:])  # Exclude intercept column for prediction
